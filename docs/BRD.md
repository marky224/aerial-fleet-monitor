...
Aerial Fleet Monitor (AFM) is a real-time information console that demonstrates the architecture and integration patterns required to surface live telemetry, detect operational anomalies, and triage them through a connected CRM. The system uses public US aviation data, and is built to mirror the patterns a fleet operations team would use to monitor a fleet of autonomous aerial assets and proactively communicate with the customers whose operations depend on those assets.
